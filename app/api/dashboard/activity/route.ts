...
export async function GET(request: NextRequest) {
  try {
    ...
    // Additional stats for Quick Stats card
    let totalReferrals = 0
    let totalOrders = 0
    let overdueReferrals = 0

    if (canViewAll) {
      ;[totalReferrals, totalOrders, overdueReferrals] = await Promise.all([
      ...
      ;[totalReferrals, overdueReferrals] = await Promise.all([
        prisma.referral.count({ where: { submittedByUserId: user.id } }),
        prisma.referral.count({
          where: {
            submittedByUserId: user.id,
            deadlineDate: { lt: new Date() },
      ...
      totalOrders = await prisma.order.count({ where: { requestorId: user.id } })
    }
